...
@Component({
  selector: 'kp-header',
  standalone: true,
  imports: [RouterLink, RouterLinkActive, RouterOutlet, NgIf, NgStyle, FormsModule],
...
export class HeaderComponent {
  ...
  onOptionChange(): void {
    if (!this.useUpper && !this.useLower && !this.useNumbers && !this.useSymbols) {
      this.useLower = true; // asegurar al menos un set
    }
    this.refreshGenerated();
  }
  ...
  copyGenerated(ev?: MouseEvent): void {
    ...
    if (navigator?.clipboard?.writeText) {
      navigator.clipboard.writeText(text)
        ...
        .catch(err => {
          console.error('[renderer] copy generated failed', err);
        });
    }
  }
